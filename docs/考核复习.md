...
| `python -m src.main weekly` | 汇总窗口内已上墙日报 → LLM 出周报 → 渲染（`--period weekly|biweekly`） | `weekly_reports` 表 | `weekly done: items=N highlights=M output=weekly.html` |
| `python -m src.main poster` | 把最新报告渲染成海报（`--period`） | — | `poster done: output=poster.html` |
...
- `run_weekly(storage, cfg, period)`：主流水线——算窗口（`_WINDOW_DAYS={weekly:7, biweekly:14}`）→ `get_surfaced_since` 取窗口内已上墙日报 → 条目 < `_MIN_ITEMS`(3) 跳过 → `_generate_weekly` 调 LLM → `save_weekly_report`（upsert）。
