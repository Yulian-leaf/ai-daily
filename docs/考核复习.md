...
并在其上叠加**周报/双周报告**（LLM 汇总已上墙日报）与**可分享海报**。
全程只有 SQLite 一个持久层，最终产物是 `index.html`（日报）/ `weekly.html` + `biweekly.html`（周报）/ `poster.html`（海报），通过 GitHub Actions 定时跑、推送到 GitHub Pages。
...
│   ├── weekly.txt          # 「周报」提示词模板（Phase 3）
│   └── biweekly.txt        # 「双周报告」提示词模板（Phase 3.5）
├── src/
│   ├── main.py             # CLI 入口：fetch / summarize / render / weekly / poster
│   ├── config.py           # 读配置 + 校验
│   ├── models.py           # 数据模型（Item/Score/Summary/Analysis/WeeklyReport）
│   ├── dedup.py            # URL 去重
│   ├── storage.py          # SQLite 读写（Storage 类）
│   ├── summarizer.py       # 评分 + 总结流水线
│   ├── weekly.py           # 周报/双周报告生成（Phase 3）
│   ├── llm.py              # LiteLLM 封装
│   ├── prompts.py          # 加载/渲染提示词
│   ├── logging_setup.py    # 日志初始化
│   ├── fetchers/           # 抓取器
│   └── notifier/
│       ├── web.py          # 渲染 HTML（render_site/render_weekly/render_poster）
│       └── labels.py       # 来源 → 友好名 + 分类
├── templates/
│   ├── index.html.j2       # 日报模板
│   ├── weekly.html.j2      # 周报/双周报告模板（Phase 3）
│   └── poster.html.j2      # 海报模板（Phase 3.5）
├── tests/                  # pytest（105 个）
...
| `python -m src.main weekly` | 汇总窗口内已上墙日报 → LLM 出周报（`--period weekly`）/ 双周报告（`--period biweekly`）→ 渲染 | `weekly_reports` 表 | `weekly done: items=N highlights=M output=weekly.html / biweekly.html` |
| `python -m src.main poster` | 把最新报告渲染成海报（`--period weekly|biweekly`） | — | `poster done: output=poster.html` |

前三个命令**共享参数** `--sources` / `--preferences` / `--db`；`render` 额外有 `--output-dir` / `--within-days`；`weekly` / `poster` 额外有 `--output-dir` / `--period`。

**幂等性**：五个命令都可以重复跑——fetch 不重复入库、summarize 不重复总结（每日封顶）、weekly 不重复生成（`UNIQUE(period, week_start)` upsert）。
...
### weekly_reports 表 —— 周报/双周报告（Phase 3 新增）

| 列 | 含义 |
| --- | --- |
| `id` (主键自增) | 报告编号 |
| `period` | `weekly` / `biweekly` |
| `week_start` / `week_end` | 覆盖起始日 / 结束日 `YYYY-MM-DD` |
| `title` | 标题（LLM 生成） |
| `slogan` | 一句话金句（海报大字展示，Phase 3.5 新增） |
| `overview` | 综述正文 |
| `highlights_json` | 亮点列表 JSON：`[{field, title, url, summary}]` |
| `trend` / `outlook` | 跨周趋势 / 下期展望 |
| `generated_at` | 生成时间 |

**幂等约束**：`UNIQUE(period, week_start)`——同周期重复生成覆盖更新，不产生重复周报。
...
- `_migrate_add_slogan()`：Phase 3.5 新增，旧库补 `weekly_reports.slogan` 列（默认 `''`）
...
- **Phase 3**：`render_weekly()` 渲染周报/双周页——按 `period` 输出 `weekly.html` / `biweekly.html`，传 `other_label`/`other_link` 做周期互跳。
- **Phase 3.5**：`render_poster()` 渲染海报——`_select_poster_highlights()` 程序化选亮点（8 领域各 1 条 + 公共池 max 2 条，带分数），传 `field_icons`（emoji）+ `field_max`（条形图归一化分母）。
...
### 4.12 `weekly.py` — 周报/双周报告生成（Phase 3）

- `run_weekly(storage, cfg, period)`：主流水线——按北京时间自然周对齐算窗口（本周一~本周日，双周再往前推一周）→ `get_surfaced_since` 取窗口内已上墙日报 → 条目 < `_MIN_ITEMS`(3) 跳过 → `_generate_weekly` 调 LLM → `save_weekly_report`（upsert）。
- `_PROMPT_NAMES`：按 period 选提示词（`weekly.txt` / `biweekly.txt`）。
- `_one_line(text, limit=80)`：把长摘要压成 ≤80 字一句话，降低 prompt 体积（防 JSON 截断）。
- `_render_weekly_prompt`：拼条目清单 + 注入 `{range}`（防标题日期幻觉）。
- `_generate_weekly`：调 `complete_json`（`max_tokens=4000`），校验 `title/overview/highlights` 必填，解析 `slogan` 组装 `WeeklyReport`。
- **输入是已上墙日报，不重新抓取/打分**，每次仅 1 次 LLM 调用。
...
### weekly.txt / biweekly.txt（Phase 3 / 3.5）

- 人设：AI4S 资深科研编辑，输入「窗口内已上墙日报条目清单 + 时间范围 `{range}`」。
- 输出严格 JSON：`{title, slogan, overview, highlights[{field,title,url,summary}], trend, outlook}`。
- 硬性要求：中文量化优先；`url` 原样照抄输入（防幻觉）；`field` 用 8 个标准 key；亮点每领域 ≤2 条、全篇 ≤12 条；标题时间范围固定用 `{range}`（防日期幻觉）。
- 双周版 `biweekly.txt` 额外要求 `trend` 做「跨周对比」（领域热度升降、新方向浮现）。
- 踩坑：初版 `max_tokens=2500` 导致输出 JSON 被截断 → 输入摘要压 80 字 + `max_tokens=4000` + 限亮点数三管齐下。
...
python -m src.main weekly --period biweekly  # 双周报告（两个自然周）
python -m src.main poster                 # 海报（默认周报）
python -m src.main poster --period biweekly  # 双周海报
